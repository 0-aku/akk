...
yes | cp /etc/letsencrypt/live/domain.com/fullchain.pem ~/domain.com/app/cert/cert.pem
yes | cp /etc/letsencrypt/live/domain.com/privkey.pem ~/domain.com/app/cert/key.pem
```

Or use akk web server for challenge exchange:

```
certbot certonly --webroot -w ~/domain.com/app/static -d www.domain.com -d domain.com -m your.name@domain.com --agree-tos --no-eff-email
```
